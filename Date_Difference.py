...
def calculate_date_difference(given_date_text):
    """Takes a date string in YYYY-MM-DD format and compare it to todays date.
       Returns a message showing if the date is in the future, past, or today.
    """
    ...
    # subtract todays date from given 
    # positive num = future date, negative number = past date.date
...
def is_response_message(file_text):
    # Avoid reading Date_Difference response as a new request.
...
def run_microservice():
    ...
    while True:
        ...
        # Only start working if the file has text and it is different from the last thing read
